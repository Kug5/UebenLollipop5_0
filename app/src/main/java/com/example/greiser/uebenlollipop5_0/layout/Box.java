...
public class Box extends ConstraintLayout {
  ...
  private void init(AttributeSet attrs, int defStyle) {
    // Load attributes
    final TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.Box, defStyle, 0);

    String tmpboxnr = a.getString(R.styleable.Box_boxnr);
    String tmpcounter = a.getString(R.styleable.Box_counter);

    inflate(getContext(), R.layout.sample_box, this);

    if (tmpboxnr != null) setBoxnr(tmpboxnr);
    if (tmpcounter != null) setCounter(tmpcounter);
  }
